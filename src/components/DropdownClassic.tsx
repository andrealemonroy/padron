...
const DropdownClassic: React.FC<FormInputProps> = ({ label, name, options, defaultSelected = 0 , validation}) => {
  ...
  // Cerrar el dropdown si se presiona la tecla "Esc"
  useEffect(() => {
    ...
    <div className="relative w-full">
      ...
      >
        <span className="flex items-center">
          <span>{options[selected - 1]?.name || 'Seleccione'}</span>
        </span>
        ...
        >
          {options.map(option => (
            <button
              ...
              onClick={() => { setSelected(option.id); setDropdownOpen(false); setValue(name, option.name);}}
